...
- `src/lib/`: Utilities (`utils.ts`, `lenis.ts`, `keystatic.ts`, `analytics.tsx`).
- `src/types/`: Shared TypeScript interfaces for Keystatic content models.
...
- **Headless CMS**: Content is read locally from `content/` using Keystatic APIs (`getAllProjects()`). In production (Cloudflare), content is read via GitHub reader.
