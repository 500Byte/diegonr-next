...
import { getAllServices, toPlainText, ExtendedService } from '@/lib/sanity';
      ...
      <section className="py-24 md:py-48">
        <SwissContainer>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-24">
            {services.map((service: ExtendedService, index: number) => (
              <FadeIn key={service.uid} delay={index * 0.2}>
                <Link href={`/services/${service.uid}`} className="space-y-8 block group">
                  ...
                  <ul className="space-y-3 pt-4">
                    {service.items?.map((item: {es?: string}, i: number) => (
